...
<form method="POST" action="{{ route('admin.testimonials.store') }}" enctype="multipart/form-data"
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="text" name="client_name" value="{{ old('client_name') }}" required class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-400">
        </div>
        <div>
            <label class="block text-sm text-gray-600 mb-1">Nama Pasangan</label>
            <input type="text" name="couple_names" value="{{ old('couple_names') }}" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-400">
        </div>
    </div>
    <div class="grid grid-cols-2 gap-4">
        <div>
    ...
    <div>
        ...
        <textarea name="content" rows="4" required class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-400">{{ old('content') }}</textarea>
    </div>
    <div>
        <label class="block text-sm text-gray-600 mb-1">Foto Klien (opsional)</label>
    ...
    <label class="flex items-center gap-2 text-sm text-gray-600">
        <input type="checkbox" name="is_published" value="1" checked class="rounded"> Tampilkan di website
    </label>
